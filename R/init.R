...
#'
#' Numeric vector.
#' It is equal to profit or loss for each asset in their currencies in the model at the moment inside backtester cycle.
...
#' one should use \code{unreal_sum}. If model has only one currence it is possible to use \code{sum(unrealized_money_last)}.
#'
#' @seealso \code{\link{unreal_sum}}
#' @export
...
#' Summed by legs unrealized pnl.
#'
#' Sum of profit and loss of each asset in model.
#' If asset has different currency from model, then pnl exchanged.
#' Often used in rule's expr for take and stop rules.
#'
#' @seealso \code{\link{unrealized_money_last}}
#'
#'@export
unreal_sum <- NULL
